...
void Main()
{
	DigitalPlumberPartOne(Input).Dump();
	DigitalPlumberPartTwo(Input).Dump();
}

int DigitalPlumberPartOne(string[] input)
{
	var programs = input.SelectMany(Parse).ToLookup(k => k.parent, e => e.child);
		
	return programs.Count(g => ContainsZero(g.Key, new HashSet<int>()));

	bool ContainsZero(int id, HashSet<int> visited)
	{
		visited.Add(id);
		return id == 0 || programs[id].Where(i => !visited.Contains(i)).Any(i => ContainsZero(i, visited));
	}
}

int DigitalPlumberPartTwo(string[] input)
{
	var programs = input.SelectMany(Parse).ToLookup(k => k.parent, e => e.child);
	var visited = new HashSet<int>();
	return programs.Count(g => IsUnvisited(g.Key));

	bool IsUnvisited(int id)
	{
		if (visited.Contains(id)) return false;
		visited.Add(id);
		return programs[id].Where(i => !visited.Contains(i)).All(IsUnvisited) || true;
	}
}

IEnumerable<(int parent, int child)> Parse(string s) 
{
	var parent = int.MinValue;
	foreach(var i in s.Split(new[] {" <-> ", ", "}, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse))
	{
