...
void Main()
{
	IHeardYouLikeRegistersPartOne(Input).Dump();
	IHeardYouLikeRegistersPartTwo(Input).Dump();
}

int IHeardYouLikeRegistersPartOne(string[] input)
{
	var processor = new Processor();
	foreach (var instruction in input.Select(i => new Instruction(i)))
	{
		processor.Execute(instruction);
	}
	return processor.GetMaxValueInRegister();
}

int IHeardYouLikeRegistersPartTwo(string[] input)
{
	var processor = new Processor();
	var maxValue = int.MinValue;
	foreach (var instruction in input.Select(i => new Instruction(i)))
	{
		var value = processor.Execute(instruction);
...
class Processor
{
	...
	public int Execute(Instruction instruction)
	{
		return (Compare(instruction)) 
			...
			: GetRegisterValue(instruction.RegisterToIncrement);
	}
	
	public int GetMaxValueInRegister()
	{
		return _registers.Values.Max();
	}
	
	private bool Compare(Instruction instruction)
	{
		var value = GetRegisterValue(instruction.RegisterToCompare);
		return instruction.Comparer(value);
	}

	private int Increment(Instruction instruction)
	{
		var value = GetRegisterValue(instruction.RegisterToIncrement);
		var newValue = instruction.Incrementer(value);
		_registers[instruction.RegisterToIncrement] = newValue;
		return newValue;
	}
...
class Instruction
{
	public string RegisterToIncrement { get; }
	public Func<int, int> Incrementer { get; }
	public string RegisterToCompare { get; }
	public Func<int, bool> Comparer { get; }

	public Instruction(string input)
	{
		var parts = input.Split(' ');

		var incrementValue = int.Parse(parts[2]);
		if (parts[1] == "dec") incrementValue = -incrementValue;

		var compareValue = int.Parse(parts[6]);

		RegisterToIncrement = parts[0];
		Incrementer = i => i + incrementValue;
		RegisterToCompare = parts[4];
		Comparer = _comparers[parts[5]](compareValue);

	}

	private static readonly Dictionary<string, Func<int, Func<int, bool>>> _comparers = new Dictionary<string, Func<int, Func<int, bool>>>
	{
		{"==", v => i => i == v},
		{"!=", v => i => i != v},
		{">", v => i => i > v},
