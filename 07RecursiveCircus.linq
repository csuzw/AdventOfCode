...
void Main()
{
	RecursiveCircusPartOne(Input).Dump();
	RecursiveCircusPartTwo(Input).Dump();
}

string RecursiveCircusPartOne(string[] input)
{
	var programs = input.Select(i => Parse(i)).ToList();
	return programs
		.Select(i => i.name)
		.Except(programs.SelectMany(i => i.childNames))
		.First();
}

int RecursiveCircusPartTwo(string[] input)
{
	var results = new List<int>();
	var programs = input.Select(i => Parse(i)).ToDictionary(k => k.name, v => (v.weight, v.childNames));
	var imbalancedProgram = programs
		.Single(p => !IsBalanced(p.Key) && p.Value.childNames.All(c => IsBalanced(c)))
		.Value
		.childNames
		.ToLookup(k => GetWeight(k), v => programs[v].weight);
	var bad = imbalancedProgram.First(i => i.Count() == 1);
	var good = imbalancedProgram.First(i => i.Count() != 1);
	return bad.First() + good.Key - bad.Key;

	bool IsBalanced(string name) => programs[name].childNames.Select(GetWeight).Distinct().Count() <= 1;
	int GetWeight(string name) => programs[name].weight + programs[name].childNames.Sum(GetWeight);
}

(string name, int weight, string[] childNames) Parse(string value)
{
	var parts = value.Split(new[] { ' ', ',', '-', '>', '(', ')' }, StringSplitOptions.RemoveEmptyEntries);
	return (parts[0], int.Parse(parts[1]), parts.Skip(2).ToArray());
}
